...
<a href="https://github.com/Octane0411/open-vibe-island/graphs/contributors">
  ...
  <img src="https://contrib.rocks/image?repo=Octane0411/open-vibe-island&t=1782735746" />
...
- **Claude Code** — Hook-based integration via `~/.claude/settings.json`. Discovers sessions from `~/.claude/projects/` JSONL transcripts. Persists and restores sessions across app launches. Managed status line bridge with opt-in installation. Reads cached 5-hour and 7-day usage windows, and for TTY-less sessions (VS Code extension, Claude Desktop) refreshes those windows via a quota-free `GET /api/oauth/usage` probe that reads Claude Code's OAuth token from the macOS Keychain (`Claude Code-credentials`, read-only, one-time authorization prompt).
- **Claude Code (Desktop App)** — The Claude desktop app runs Claude Code in its "local agent mode" as a TTY-less subprocess that `ps`/`lsof` process discovery cannot see. Detected via `CLAUDE_CODE_ENTRYPOINT=claude-desktop` (with `__CFBundleIdentifier=com.anthropic.claudefordesktop` as a fallback) and tagged with a `Claude.app` jump target so liveness follows the running desktop app (`NSWorkspace.shared.runningApplications`) instead of the missing terminal process. Without this, the hook-managed liveness fallback evicted the session ~6 seconds after it appeared ([#510](https://github.com/Octane0411/open-vibe-island/issues/510)). Clicking a session activates Claude. **Usage caveat:** the 5h/7d usage panel is normally fed by Claude Code's terminal status line, which Claude Desktop's headless `stream-json` mode never renders. To cover this, Open Island polls Claude's dedicated, quota-free usage endpoint (`GET https://api.anthropic.com/api/oauth/usage`) whenever a Desktop (or VS Code extension) session is live and the cached quota snapshot is stale (>120s) — using the OAuth access token it reads read-only from the macOS Keychain item `Claude Code-credentials`. The first read triggers a one-time macOS authorization prompt ("Open Island wants to use ... Claude Code-credentials"); Open Island never refreshes or rotates the token. If that prompt is denied, the panel simply won't update (fails open, no error) and falls back to whatever the terminal status line last seeded, since the limits are account-wide.
...
Claude usage setup is available from the app's Settings window and remains opt-in. The bridge writes a managed `statusLine.command` to `~/.open-island/bin/open-island-statusline`, caches `rate_limits` into `/tmp/open-island-rl.json`, and refuses to overwrite an existing custom status line automatically. For TTY-less sessions (VS Code extension, Claude Desktop) that never render the status line, Open Island falls back to polling Claude's quota-free `/api/oauth/usage` endpoint using the Keychain-stored OAuth token — see the usage caveat under Agent Integrations above.
